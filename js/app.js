...
(function () {
  ...
  // Single-sided boards are commonly drawn as seen from the copper (solder)
  // side, so their silkscreen text — stored as glyph-stroke geometry — comes
  // out mirrored in a normal top view. Detect that and mirror by default so
  // the text is readable immediately; X flips back to the raw orientation.
  //
  // The reliable signal is the copper distribution: all copper on the bottom
  // layer (C2) with none on top (C1) and no inner-layer copper means a
  // single-sided, copper-side design. (An earlier glyph-order heuristic was
  // unreliable on real files, where component children are strokes in an
  // order that does not track reading direction.)
  function looksMirrored(board) {
    if (board._mirrorDet !== undefined) return board._mirrorDet;
    var lc = Lay6.layerCounts(board);
    var topCopper = lc[1], botCopper = lc[3], innerCopper = lc[5] + lc[6];
    board._mirrorDet = botCopper > 0 && topCopper === 0 && innerCopper === 0;
